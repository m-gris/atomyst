...
let manifest_yaml ~source_name ~definitions =
  ...
  List.iter (fun (d : definition) ->
    let filename = Snake_case.to_snake_case d.name ^ ".py" in
...
let manifest_json ~source_name ~definitions =
  let defs =
...
let manifest_md ~source_name ~definitions =
  let buf = Buffer.create 512 in
...
let manifest ~format ~source_name ~definitions =
  match format with
  | "json" -> (manifest_json ~source_name ~definitions, "MANIFEST.json")
  | "md" -> (manifest_md ~source_name ~definitions, "MANIFEST.md")
  | _ -> (manifest_yaml ~source_name ~definitions, "MANIFEST.yaml")
